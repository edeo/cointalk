...
We can see the total number of none stop words Aaron and Jay have used:
...
Using the Afinn table, there is a different trend, showing Jay take a more positive  
...
```{r, echo=FALSE,message=FALSE}
 ...
 fear_graph_df <- convo_w_ncr %>% filter(nrc_sentiment=='fear') %>% group_by(name,episode_number,nrc_sentiment) %>% tally()
...
anger_graph_df <- convo_w_ncr %>% filter(nrc_sentiment=='anger') %>% group_by(name,episode_number,nrc_sentiment) %>% tally()
